...
완전한 독립성 보장: 각자의 코드(linear.c, nonlinear.c 등) 간에 어떠한 함수 호출이나 의존성도 없어야 합니다. 오직 본인에게 할당된 알고리즘 로직에만 집중합니다.

공용 파일 수정 금지: matrix.c 및 matrix.h 파일은 절대 임의로 건드리지 않습니다. 기능 추가나 수정이 필요할 경우 반드시 조장에게 건의합니다.

👥 팀원 및 담당 주제
최지훈: LU Decomposition with Partial Pivoting (부분 피벗 LU 분해)

전혜빈: Conjugate Gradient Method (켤레기울기법)

이항석: Multiderivative(Multivariate) Newton-Raphson (다변수 뉴턴-랩슨 법)

김민교: Broyden's Method (브로이든 법)

안의빈: Power Iteration, Inverse Power Iteration (거듭제곱법 및 역거듭제곱법)

박세영 (조장): QR Algorithm with Shifts (QR 알고리즘)

📖 matrix.c API 사용 설명서
조원들은 자신의 .c 파일 최상단에 #include "matrix.h"를 선언한 뒤, 아래의 함수들만 사용하여 솔버를 구현해야 합니다.
...
3. 사용 가능 함수 목록 (API Reference)
🏗️ 생성 및 파괴 (Creation & Destruction)
C
...
📐 벡터 및 혼합 연산 (Vector & Mixed Operations)
C
